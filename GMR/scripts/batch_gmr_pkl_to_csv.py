...
if __name__ == "__main__":
    ...
    for i, file in enumerate(os.listdir(args.folder)):
        ...
        # GMR -> whole_body_tracking
        # 1) root_pos: empirical fix from replay results, swap y/z
        motion[:, :3] = motion_data["root_pos"][:, [0, 2, 1]]

        # 2) root_rot: GMR stores wxyz, whole_body_tracking expects xyzw in CSV
        motion[:, 3:7] = motion_data["root_rot"][:, [1, 2, 3, 0]]
        motion[:, 7:] = dof_pos

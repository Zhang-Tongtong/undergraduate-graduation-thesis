...
class UrdfConverter(AssetConverterBase):
    ...
    def __init__(self, cfg: UrdfConverterCfg):
        ...
        # 先尝试列出所有跟 urdf importer 相关的扩展
        all_exts = manager.get_extensions()
        urdf_ext_ids = [ext["id"] for ext in all_exts if "isaacsim.asset.importer.urdf" in ext["id"]]

        print("[patch] found URDF extensions:", urdf_ext_ids)

        # 优先启用精确版本；如果没找到，就启用所有匹配项
        if urdf_ext_ids:
            for ext_id in urdf_ext_ids:
                if not manager.is_extension_enabled(ext_id):
                    print(f"[patch] enabling extension: {ext_id}")
                    manager.set_extension_enabled_immediate(ext_id, True)
        else:
            # 兜底：试无版本号名字
            fallback_id = "isaacsim.asset.importer.urdf"
            try:
                if not manager.is_extension_enabled(fallback_id):
                    print(f"[patch] enabling fallback extension: {fallback_id}")
                    manager.set_extension_enabled_immediate(fallback_id, True)
            except Exception as e:
                print("[patch] fallback enable failed:", repr(e))

        from isaacsim.asset.importer.urdf._urdf import acquire_urdf_interface

        self._urdf_interface = acquire_urdf_interface()
        print("[patch] acquire_urdf_interface ok")
